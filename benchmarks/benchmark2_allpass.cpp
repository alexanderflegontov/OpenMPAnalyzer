...
std::pair<double, std::vector<std::string>> Sequent_hack(const char* hash, const size_t MinCodeLen, const size_t MaxCodeLen, const std::vector<std::string> &alphabets)
{
...
//function_exit:
    ...
    return std::make_pair(end_time, vec_pwds);
}
...
std::pair<double, std::vector<std::string>> Parallel_hack(const char* hash, const size_t MinCodeLen, const size_t MaxCodeLen, const std::vector<std::string> &alphabets, const uint64_t block_size)
{
...
*/
    ...
    for(auto i = 0; i < totalNumThreads; ++i)
    {
        ...
        {
            vec_pwds.insert(vec_pwds.end(), vec_pwds_per_thread[i].begin(), vec_pwds_per_thread[i].end());
        }
    }
    return std::make_pair(end_time, vec_pwds);
}
...
int main(int argc, char* argv[]){
    ...
    if(CheckValidHash(hash))
    {
        ...
        if(IsFound){
            ...
            std::cout << " Parallel_hack found the passwords: " << std::endl; 
            for(auto i = 0; i < vect_pwdParall.size(); ++i)
            {
                std::cout << "passwordPar[" << i << "]: " << vect_pwdParall[i] << std::endl;
            }
        }else{
        ...
        if(test_mode){
            ...
            if(IsFound){
                ...
                std::cout << " Sequent_hack found the passwords: " << std::endl; 
                for(auto i = 0; i < vect_pwdSequent.size(); ++i)
                {
                    std::cout << "passwordSeq[" << i << "]: " << vect_pwdSequent[i] << std::endl;
                }
            }else{
                std::cout << " Not Found!" << std::endl;
            }
            ...
            std::unordered_set<std::string> passSeqSet(vect_pwdSequent.begin(), vect_pwdSequent.end());
            bool isOneFound = false;
            for(auto item : vect_pwdParall)
            {
                if(passSeqSet.count(item) == 0)
                {
                    std::cout << item << " - not found in sequent hack." << std::endl;
                }
                else
                {
                    isOneFound = true;
