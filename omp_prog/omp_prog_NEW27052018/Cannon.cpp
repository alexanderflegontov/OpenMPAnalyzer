...
int main(int argc, char* argv[])
{
	...
	std::cout << "numTh = " << numTh << std::endl;

	int gridSize = int(sqrt((double)numTh));
	if(!(numTh >=4 && gridSize*gridSize == numTh)){
		std::cout <<"numTh should be 4,9,16, e.t.c " << std::endl;
		std::cout << "<<example: numTh*numTh == sqrt(numTh)" <<std::endl;
		return 1;
	}
	...
	std::cout << "MATRIX dimension: " << n_tmp << "x" << n_tmp << std::endl;
	if(!(n_tmp>0)){
		std::cout <<"MATRIX dimension should be > 0 " << std::endl;
		return 2;
	}
